...
try:
    with warnings.catch_warnings():
        ...
        from .tts import ChatterboxOfficial23LangTTS
    TTS_AVAILABLE = True
except ImportError:
    TTS_AVAILABLE = False
    # Create dummy class
    class ChatterboxOfficial23LangTTS:
        ...
        def from_pretrained(cls, device):
            raise ImportError("ChatterboxOfficial23LangTTS not available - missing dependencies")
        @classmethod
...
try:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        from .vc import ChatterboxOfficial23LangVC
    VC_AVAILABLE = True
except ImportError:
    VC_AVAILABLE = False
    # Create dummy class
    class ChatterboxOfficial23LangVC:
        @classmethod
        def from_pretrained(cls, device):
...
try:
    ...
    __all__ = [
        'ChatterboxOfficial23LangTTS', 'ChatterboxOfficial23LangVC',
        'get_official_23lang_models', 'get_model_config', 'get_model_files_for_language',
        'find_local_model_path', 'detect_model_format', 'get_available_languages',
        'is_model_incomplete', 'get_model_requirements', 'validate_model_completeness',
        'get_tokenizer_filename',
